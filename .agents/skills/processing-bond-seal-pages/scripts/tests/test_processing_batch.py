...
class FakeWordConverter:
    ...
    def convert(self, working_paper_path, output_path):
        working_paper_path = Path(working_paper_path)
        output_path = Path(output_path)
        self.calls.append(working_paper_path.name)
        if working_paper_path.name == self.failed_name:
    ...
    def test_prepares_batch_without_deduplicating_and_continues_after_failure(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            for name, content in files.items():
                (working_paper_root / name).write_bytes(content)
            (working_paper_root / "忽略.txt").write_text("不是 Word", encoding="utf-8")
            working_paper_hashes = {
                name: _sha256(working_paper_root / name) for name in files
            }
            ...
            self.assertEqual(
                [item["working_paper_path"] for item in ready],
                ["01-甲.docx", "02-乙.doc"],
            )
            ...
            self.assertTrue(all(len(item["working_paper_sha256"]) == 64 for item in ready))
            ...
            self.assertEqual(failed[0]["working_paper_path"], "03-失败.docx")
            self.assertIn("模拟转换失败", failed[0]["error"])
            self.assertEqual(
                {_sha256(working_paper_root / name) for name in files},
                set(working_paper_hashes.values()),
            )
