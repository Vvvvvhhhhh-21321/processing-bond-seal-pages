...
安装后，在智能体客户端的**聊天框**中调用 Skill，不是在 PowerShell 或终端中运行。客户端支持 `$` 调用时，以 `$processing-bond-seal-pages` 开头；不支持时，改为“请使用 processing-bond-seal-pages Skill”。其余内容直接用自然语言描述即可，不需要记住 `prepare`、`date-review` 或 `finalize` 等内部命令。

文件位置可以直接写本地路径。客户端支持附件引用时，已有的单个回章 PDF 可以直接 `@文件`；当前版本的 Word 批次输入必须是一个文件夹，新输出目录也尚不存在，因此这两类位置建议直接写目录路径。

### 1. 生成待盖章页合集

```text
$processing-bond-seal-pages
生成待盖章页合集。底稿目录：“<底稿文件目录>”；处理批次输出到：“<处理批次目录>”。
```

Skill 会接着询问：

> 保留全部 Word，还是按 Word 文件哈希排除重复文件？

回答“保留全部”或“按哈希排除重复文件”即可。在你回答前，Skill 不会转换 Word 或自行选择默认值。随后它会检查环境；如果需要安装，会先说明影响并等待同意。
...
```text
$processing-bond-seal-pages
生成日期确认稿。处理批次：“<处理批次目录>”；回章 PDF：@回章页合集.pdf；日期：2026-07-15；输出到：“<日期确认目录>”。
```

如果客户端不支持 `@文件`，把它换成回章 PDF 的完整路径。Skill 会完成匹配和落日期，在生成确认稿后自动暂停，不会直接回拼。
...
```text
$processing-bond-seal-pages
我已检查并确认日期确认稿。处理批次：“<处理批次目录>”；日期确认目录：“<日期确认目录>”；请回拼到：“<输出目录>”。
```

Skill 会复用已保存的匹配关系完成回拼，并生成 HTML 处理清单。

## 高级用法：直接运行脚本

普通用户不需要运行下面的命令。排查环境或调试时，可以直接使用统一入口：
